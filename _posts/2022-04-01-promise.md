...
- 1. return promise `인스턴스` : promise 인스턴스가 리턴된 것<br/>
  - return된 애도 promise
  - 실행결과가 언젠가 pending에서 resolve로 바뀜
  - 그럼 다시 그 다음 **then을 탈 수 있음**
- 2. return `일반값` : promise 객체에 resolved 상태로 반환. 그 안에 값이 담김<br/>
  - Promise { <resolve>:  값 } 이렇게
- 3. return 안하면 : return undefined (원래 JS 동작이 이러함)
  - undefined가 곧 일반값. = 2번과 같다
- 4. Promise.resolve() 또는 Promise.reject()
  - return하지 않는 이상 의미없음!
